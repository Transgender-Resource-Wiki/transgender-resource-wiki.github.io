---
title: Travelling
layout: default
nav_order: 4
has_children: false
---

# Travelling as a Transgender Person

Unfortunately, as transgender people, we have to watch out for our safety and do plenty of research before we travel. This page contains links to several websites that will help you out if you are planning on travelling. While this page is a great place to start, it contains more general information. You should also do your own research to find out the specifics of the place you are travelling to.

<div class="warning" style='background-color:#e8a095; color:#a32315; border-left: solid #a32315 4px; border-radius: 12px; padding:0.7em;'>
    <span>
        <p style='margin-left:0.5em; text-align:left'> <b>DO NOT TRAVEL WARNINGS: </b></p>
        <p style='margin-left:1em; text-align:left'>
        Absolutely do not travel to the following places for any reason. Also note that just because a place is not listed here that doesn't mean it is safe. These are just the most dangerous places. Refer to the resources below to see every countries' safety level.
        </p>
        <ul>
            <li>Entire Middle East</li>
            <li>Most of Africa, Asia, & Oceania</li>
            <li>The Caribbean</li>
            <li>United States: 
                <ul>
                    <li>Absolutely do not travel to Florida</li>
                    <li>Avoid all other red/rural states</li>
                </ul></li>
            <li>Eastern Europe</li>
            <li>Cambodia</li>
            <li>El Salvador</li>
            <li>Guatemala</li>
            <li>Guyana</li>
            <li>Honduras</li>
            <li>Panama</li>
            <li>Paraguay</li>
            <li>Philippines</li>
            <li>Venezuela</li>
        </ul>
    </span>
</div>

## Where is safe?

<p>Wherever you decide to go, try to stick to cities, as the people there tend to be more progressive and accepting than in the countryside. Some safe places you can travel to include the United States (Blue states only; avoid connecting flights in red states), Canada, Western & Northern Europe, Australia, and New Zealand.</p> 
In the following places you will probably be fine, but will need to be cautious about certain things. People may stare, especially older people, but violence is very unlikely; try to stick to tourist areas. Japan (avoid onsen w/o bottom surgery), the UK (AKA FART Island[^1]), Taiwan, Thailand, & Malta.

## Resource Links

[Refuge Restrooms]&#58; Search for local restrooms and get a rating on how safe they are. An app version is also available.

[World Transgender Rights Index]&#58; Provides a ranking for each country on how safe it is.

[LGBTQ World Travel Safety Map]&#58; Note that some places are rated safer than they might actually be for transgender people. Since gay safety is also factored in it can raise the scores of places that are safe for gay people but not so much for transgender people.

[Travel while Trans]&#58; Information on laws affecting transgender people. Covers Canada, Mexico, New Zealand, and USA. 

### USA

[Erin in the Morning's Trans Legislative Risk Map]&#58; Map of states with protections and states with anti-trans laws.

[USA LGBT Restroom Map]&#58; Map of states where using the "wrong" restroom is illegal.

[USA LGBT Equality Map]&#58; Make sure to select Gender Identity tab to view map for transgender equality specifically.

[Airport Security FAQ]&#58; Answers many common questions you may have about airport security.

[TSA "Cares"]&#58; Official TSA instructions for transgender travellers.

[USA International LGBTQ Travel Guide]&#58; Official USA government guide/advisories for LGBTQ people travelling internationally.



[^1]: FART = Feminist Appropriating Radical Transphobe

[Refuge Restrooms]: https://www.refugerestrooms.org/


[World Transgender Rights Index]: https://www.asherfergusson.com/global-trans-rights-index/
[LGBTQ World Travel Safety Map]: https://www.asherfergusson.com/lgbtq-travel-safety/
[Travel while Trans]: https://travelwhiletrans.com/#/

[Erin in the Morning's Trans Legislative Risk Map]: https://www.erininthemorning.com/p/june-anti-trans-legislative-risk
[USA LGBT Restroom Map]: https://www.lgbtmap.org/equality-maps/nondiscrimination/bathroom_bans
[USA LGBT Equality Map]: https://www.lgbtmap.org/equality-maps

[Airport Security FAQ]: https://transequality.org/know-your-rights/airport-security
[TSA "Cares"]: https://www.tsa.gov/travel/tsa-cares/gender-diversity
[USA International LGBTQ Travel Guide]: https://travel.state.gov/content/travel/en/international-travel/before-you-go/travelers-with-special-considerations/lgbtqi.html
